...
class PluginsConfigFactory {
    ...
    static createFromNodeConfig(nodeConfig, projectRootDir) {
        ...
        const hapiRouterConfigDefaults = {
            enabled: true,
            routes: 'app/server/routes/**/*Routes.js'
        };
        let hapiRouterConfig = hapiRouterConfigDefaults;
        if (nodeConfig.has('server.plugins.hapiRouter')) {
            hapiRouterConfig = _.merge(hapiRouterConfigDefaults, nodeConfig.get('server.plugins.hapiRouter'));
        }
        hapiRouterConfig.cwd = projectRootDir;
