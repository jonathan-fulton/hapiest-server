...
class Internals {
    ...
    static addGoodPlugin(config, plugins, logger) {
        if (config.enabled) {
            const reporters = [new GoodWinston({
                    request: '*',
                    error: '*'
                }, logger.getLogger())
            ];
            if(config.consoleReporterEnabled) {
     ...
     * @param {Array} plugins
     *
     * @TODO: refactor so that routes is always relative to project root, not CWD
     */
